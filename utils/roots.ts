...
const ROOT_SEARCH_CACHE = new Map<string, string[]>();
...
export const getArabicRoot = (word: string): string => {
    if (!word) return '';
    const cached = ROOT_CACHE.get(word);
    if (cached !== undefined) return cached;

    const res = computeArabicRoot(word);
    ROOT_CACHE.set(word, res);
    return res;
};
...
export const findWordsByRoot = (query: string, quranWords: Set<string> | string[]): string[] => {
    ...
    if (ROOT_DICTIONARY_OVERRIDE[queryRoot]) {
        ROOT_SEARCH_CACHE.set(cleanQuery, ROOT_DICTIONARY_OVERRIDE[queryRoot]);
        return ROOT_DICTIONARY_OVERRIDE[queryRoot];
    }
    if (ROOT_DICTIONARY_OVERRIDE[cleanQuery]) {
        ROOT_SEARCH_CACHE.set(cleanQuery, ROOT_DICTIONARY_OVERRIDE[cleanQuery]);
    ...
    const result = Array.from(matchedWords);
    ROOT_SEARCH_CACHE.set(cleanQuery, result);
    return result;
};
